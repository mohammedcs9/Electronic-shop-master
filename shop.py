...
root.title('Market tools for Building [متجر ادوات البناء]')
...
def save():
    name = En_name.get()
    phone = En_phone.get()
    address = En_adress.get()
    total = En_total.get()
    datebuy = En_date.get()
    
    excel = openpyxl.load_workbook('mohmd.xlsx')
    file = excel.active
...
def bill():
    ...
    L_adress = Label(F4, text='عنوان المشتري' , bg='#5F7161',fg='white')
    L_adress.place(x=160,y=130)
    En_adress = Entry(F4,width=24,font=('Tajawal',12),justify=CENTER)
    En_adress.place(x=15,y=160)
...
title = Label(F1, text='مشروع بيع معدات البناء', font=('Tajawal 13'), fg='white', bg='#5F7161', width=70)
title.place(x=0,y=0)
...
menu7 = Button(F1, width=88, bg='#EFEAD8', bd=1, relief=SOLID, cursor='hand2', height=85, image=img_menu7, text='قيعة امان', compound=TOP)
...
b2 = Button(F1,text='فاتورة جديدة' ,fg='white',font=('Tajawal 12'), width=15, bg='#6D8B74', bd=1, relief=SOLID, cursor='hand2', height=1,command=clear)
...
b4 = Button(F1,text='اغلاق البرنامج' ,fg='white',font=('Tajawal 12'), width=15, bg='#6D8B74', bd=1, relief=SOLID, cursor='hand2', height=1)
